...
| 2 | `/create` → capacity proof → terms | The browser proves the threshold locally, checks the proof through a Soroban simulation, and the form sets the campaign's minimum funding threshold |
...
| 5 | `/explore` and `/kampanya/<id>` | Campaign state read from the configured contracts: the minimum marked on the funding bar, the advance drawn so far and the actions the current state allows |
...
arrives. At `10,000`, the campaign is all-or-nothing. The creation form offers
50%, 60%, 75% and all-or-nothing; the contract accepts any value from 1 to
10,000 basis points and rounds the resulting minimum up. Contributions cannot
exceed the target or arrive after the deadline.
...
**Investors claim after settlement.** The farmer owes only what was actually
drawn: `amount_due` is the drawn principal plus `return_bps` on that principal,
not on the target. Repayment adds that amount to a fixed pool, which investors
withdraw pro rata through `claim`; a second claim is refused. Contributions are
accounting entries, not transferable investment tokens.
...
through the anchor sandbox. A campaign page marks the minimum on the funding
bar, shows the advance drawn so far, and offers the farmer the drawdown and
close-funding actions its current state allows.
...
`create_campaign` takes the capacity claim and proof, the public crop and region
labels, the target, the deadline, `return_bps` and `min_bps`. `min_bps` is the
ninth argument and carries the minimum funding threshold in basis points; a
client built against the earlier all-or-nothing interface fails the call.

| Status | Meaning | What can happen next |
|---|---|---|
| `Funding` | Open for contributions, escrow held as vault shares | `fund`; `disburse` once the minimum is met; `close_unfunded` after the deadline if it never was |
| `Funded` | Target reached, not all of it drawn yet | `disburse` |
| `Disbursed` | Funding closed and everything raised paid out | `repay` |
| `Repaid` | Farmer repaid the drawn principal plus the return | `claim` |
| `Refunding` | Minimum missed, vault position unwound | `claim` |

| Function | Caller | Effect |
|---|---|---|
| `create_campaign` | Farmer | Verifies the capacity proof, spends the nullifier and opens the campaign |
| `fund` | Investor | Moves USDC in and deposits it into the vault |
| `disburse` | Farmer | Pays out what is raised and not yet drawn; callable again as more arrives; moves to `Disbursed` once the target is reached or the deadline has passed, even with nothing new to pay |
| `close_unfunded` | Anyone | After the deadline and only below the minimum: unwinds the vault, releases the nullifier, moves to `Refunding` |
| `amount_due` | Anyone | Read-only: the drawn principal plus `return_bps` on it |
| `repay` | Farmer | Pays that amount in, fixes the investor pool and raises the anonymous reputation tier |
| `claim` | Investor | Pro-rata share of that pool, once per investor |
| `get_campaign`, `campaign_count`, `investment_of`, `has_claimed`, `reputation_of`, `quoted_rate_bps`, `config` | Anyone | Read-only |

Failures surface as contract error codes; the interface maps them to Turkish and
English messages.

| Code | Name | Raised when |
|---|---|---|
| 1 / 2 | `AlreadyInitialized` / `NotInitialized` | Setup called twice, or a call before initialization |
| 3 | `NotFound` | No campaign with that id |
| 4 | `NullifierAlreadyUsed` | That attestation already opened a campaign this season |
| 5 | `InvalidAmount` | Non-positive contribution |
| 6 | `WrongStatus` | The action does not match the campaign's current status |
| 7 / 8 | `DeadlineNotReached` / `DeadlinePassed` | Timing |
| 9 | `ExceedsTarget` | The contribution would take the campaign past its target |
| 10 / 11 | `NothingToClaim` / `AlreadyClaimed` | A claim by a non-investor, or a second claim |
| 12 | `InvalidParameters` | Non-positive target, `return_bps` above 10,000, or `min_bps` outside 1-10,000 |
| 14 | `BelowMinimum` | Drawdown attempted before `min_bps` of the target is raised |
...
| `node scripts/partial-funding-on-testnet.mjs` | Testnet transactions | A prepared campaign half funded, drawn, funded again, drawn again, repaid and claimed |
...
investor claims depend on repayment. Settlement also depends on the farmer
acting: once the minimum is met the refund path is closed, so a campaign whose
farmer never calls `disburse` stays in `Funding` and its investors cannot claim.
There is no automatic relationship between the capacity threshold and permitted
funding size. Return basis points describe the campaign's agreed return, not a
measured vault APY.
...
which the frontend imports at build time. The campaign entry is the
minimum-threshold contract; the all-or-nothing one it replaced is kept under
`legacy`. A stale local copy of `deployments.json` points the interface at that
old contract.
...
When only the campaign contract changes, as it did for `min_bps`,
[`scripts/deploy-campaign.mjs`](scripts/deploy-campaign.mjs) deploys and
initializes a new one against the existing verifier, vault and token, and moves
the previous address under `legacy` in the manifest. Campaigns do not move with
it: they have to be opened again on the new contract and their ids restart, so
an id noted from an earlier deployment no longer refers to the same campaign.
...
`scripts/partial-funding-on-testnet.mjs` walks the minimum-threshold path on
the live contract: half the target funded, the farmer draws while funding is
still open, the rest arrives, the farmer draws again, repays, and an investor
claims. It takes the newest rehearsal campaign from `.harvest/demo-farmers.json`,
which `scripts/open-demo-campaigns.mjs` writes; that helper is referenced by the
committed scripts but is not itself in the repository, so this is not a fresh
install smoke command. Partial funding is also covered by the Rust campaign
tests and can be exercised through the UI.
