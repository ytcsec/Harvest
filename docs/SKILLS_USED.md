...
*"verification gateway → policy layer → application logic"* is why this is three
contracts rather than one: `harvest-verifier` (crypto + accreditation policy),
`harvest-campaign` (state transitions), and a vault referenced only by address.
