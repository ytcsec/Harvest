...
export async function proveCapacity({
  ...
  if (threshold > yieldKg) {
    ...
    throw new Error(
      `The threshold (${threshold} kg) is above the attested yield (${yieldKg} kg). ` +
        `The circuit will not produce a proof for a claim that is not true.`,
    );
  }
  ...
  if (!(await snarkjs.groth16.verify(vkey, publicSignals, proof))) {
    throw new Error("the proof failed local verification -- refusing to submit it");
  }
...
function assertNothingLeaked(payload, secrets) {
  ...
  for (const secret of secrets) {
    if (serialized.includes(secret.toString())) {
      throw new Error("a private value appeared in the on-chain payload -- aborting");
    }
  }
}
