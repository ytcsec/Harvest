 ...
 *
 * Ported from the earlier Vite app (removed; see git history), plus the rest
 * of the campaign lifecycle (disburse, repay, claim, close_unfunded) that the
 * campaign detail page drives.
 *
...
async function issuerJson(path, init) {
  ...
  if (!res.ok) {
    const err = new Error(
      body.error ?? tl(`kooperatif servisi ${res.status} döndü`, `cooperative service returned ${res.status}`),
    );
    // `NOT_ENROLLED` is not a fault: the caller shows it as "register first".
    err.code = body.code;
...
// There is no `fetchMembers`. The endpoint it called published every grower's
// expected yield to anyone who asked -- the figure the proofs exist to keep from
// the market. A farmer's record now comes back one at a time, below, and only to
// the commitment that enrolled for it.

/**
 * This browser's own member record, or `{ membershipId: null }`.
 *
 * The cooperative already holds this mapping -- it signed the credential -- so
 * asking costs no privacy, and it saves a farmer from picking someone else's
 * membership off the list and being refused.
 */
export const fetchMyMembership = (farmerCommitment) =>
  issuerJson("/my-membership", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ farmerCommitment }),
  });
