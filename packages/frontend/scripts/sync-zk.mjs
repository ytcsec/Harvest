...
const files = [
  ...
  // The membership-enrolment circuit. Fetched only when a farmer registers, so
  // an investor never pays for it.
  [join(build, "harvest_enrolment_js", "harvest_enrolment.wasm"), "harvest_enrolment.wasm"],
  [join(build, "harvest_enrolment_final.zkey"), "harvest_enrolment_final.zkey"],
  [join(build, "harvest_enrolment_verification_key.json"), "harvest_enrolment_verification_key.json"],
];
